...
;; /usr/bin/emacs
;; 10-perlnow-guess-run-string-with-module-starter.t
...
(defun et-greasy-monkees-warp-the-fundament ()
  ...
  (perlnow-tron)
  (let* (
         (file-prefix "10") ;; Someday, in lost perly fields, I will find $0 again
         (sub-directory (concat "buuq" file-prefix))
         ...
         (mod-style   "modstar") ;; noncpan, modstar, h2xs...
         (test-ext-from  "") ;; pl, pm, or t
         (test-ext-new   "") ;; pl, pm, or t
         (test-mess      "") ;; for "new", from "from", in "context"
         (perl "/usr/bin/perl")
         ;; module-starter (with Module::Build)
         (package-name "Planet::Ten")
         ...
         (exp-pm-file
          (concat staging-area "lib"
                  perlnow-slash "Planet" perlnow-slash exp-pm-base))
    ...
    ;; "MODSTAR  aka module-starter
    (if perlnow-debug (message (upcase mod-style)))

    ;; For a new module-starter project,
    ...
    (assert-t
     ...
     "Testing that perlnow-module-starter left new pm buffer active.") ;; ok 1
    ...
    (assert-equal exp-t t-file
                  (concat "Testing module-starter run created test file: \n"
                          "        " t-file )) ;; ok 2
    ...
    (setq exp-rs-from-pm
          (concat
           "perl " staging-area "t/01-Planet-Ten.t"))
    (setq rs-from-pm (perlnow-guess-run-string))
    (if perlnow-debug (message "rs-from-pm post-pl, after t again: %s" rs-from-pm))
    (assert-equal exp-rs-from-pm rs-from-pm
